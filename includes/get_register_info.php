						...
						foreach ($fields["$formid"]["$pfcid"] AS $displayorder => $profilefields)
						{
							...
							foreach ($profilefields AS $_profilefieldid => $profilefield)
							{
								...
								if ($profilefield['type'] == 'input')
								{
									...
									$custom_field_holder = '<input type="text" class="primary textbox" name="userfield['. $profilefieldname .']" id="cfield_'. $profilefield['profilefieldid'] .'" value="'. $profilefield['currentvalue'] .'" maxlength="' . $profilefield['maxlength'] . '" tabindex="1" /><p class="description">' . $profilefield['description'] . '</p><input type="hidden" name="userfield[' . $profilefieldname . '_set]" value="1" />';
								}
								elseif ($profilefield['type'] == 'textarea')
								{
									...
									$custom_field_holder = '<div><textarea class="primary textbox" name="userfield[' . $profilefieldname . ']" id="cfield_' . $profilefield['profilefieldid'] . '" rows="' . $profilefield['height'] . '" cols="60" tabindex="1">' . $profilefield['currentvalue'] . '</textarea></div><p class="description">'. $profilefield['description'] . '</p><input type="hidden" name="userfield['. $profilefieldname . '_set]" value="1" />';
								}
...
$show['email'] = ($options['enableemail'] AND $options['displayemails']) ? true : false;
if ($_POST['timezoneoffset'])
{
	$timezonesel = $_POST['timezoneoffset'];
}
else
{
	$timezonesel = $options['timeoffset'];
}
$timezoneoptions = '';